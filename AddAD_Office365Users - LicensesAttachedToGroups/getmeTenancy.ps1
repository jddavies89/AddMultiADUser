...
.DESCRIPTION    
    ...
    Functions to edit before running the script for the first time:
        ...
        Also everywhere contoso is, add your tenancy.
...
function updateAllProPlusFacultyUsr($user){
   try{
        ...
        $O365All = New-MsolLicenseOptions -AccountSkuId  contoso:OFFICESUBSCRIPTION_FACULTY
...
function updateAllProPlusStudentUsr($user){
   try{
        ...
        $O365All = New-MsolLicenseOptions -AccountSkuId  contoso:OFFICESUBSCRIPTION_STUDENT
        Set-MsolUserLicense -UserPrincipalName $user -LicenseOptions $O365All
...
function updateAllEduFacultyUsr($user){
   try{
        ...
        $O365All = New-MsolLicenseOptions -AccountSkuId  contoso:STANDARDWOFFPACK_FACULTY
        Set-MsolUserLicense -UserPrincipalName $user -LicenseOptions $O365All
...
function updateAllEduStudentUsr($user){
   try{
       ...
       $O365All = New-MsolLicenseOptions -AccountSkuId  contoso:STANDARDWOFFPACK_STUDENT
       Set-MsolUserLicense -UserPrincipalName $user -LicenseOptions $O365All
...
function addAllProPlusFacultyUsr($user){
  try{
       ...
       $O365AllFac = New-MsolLicenseOptions -AccountSkuId  contoso:OFFICESUBSCRIPTION_FACULTY
       Set-MsolUserLicense -UserPrincipalName $user -AddLicenses  contoso:OFFICESUBSCRIPTION_FACULTY -LicenseOptions $O365AllFac  
...
function addAllProPlusStudentsUsr($user){
    try{
       ...
       Set-MsolUser -UserPrincipalName $user -UsageLocation GB
       $addO365AllStu = New-MsolLicenseOptions -AccountSkuId  contoso:OFFICESUBSCRIPTION_STUDENT
       Set-MsolUserLicense -UserPrincipalName $user -AddLicenses  contoso:OFFICESUBSCRIPTION_STUDENT -LicenseOptions $addO365AllStu
...
function addAllEduFacultyUsr($user){
  try{
        ...
        Set-MsolUser -UserPrincipalName $user -UsageLocation GB
        $O365All = New-MsolLicenseOptions -AccountSkuId  contoso:STANDARDWOFFPACK_FACULTY
        Set-MsolUserLicense -UserPrincipalName $user -AddLicenses contoso:STANDARDWOFFPACK_FACULTY -LicenseOptions $O365All
...
function addAllEduStudentUsr($user){
    try{
       ...
       Set-MsolUser -UserPrincipalName $user -UsageLocation GB
       $O365All = New-MsolLicenseOptions -AccountSkuId contoso:STANDARDWOFFPACK_STUDENT
       Set-MsolUserLicense -UserPrincipalName $user -AddLicenses  contoso:STANDARDWOFFPACK_STUDENT -LicenseOptions $O365All
...
function addSellectedProPlusFacultyUsr($user){
    try{
       ...
       Set-MsolUser -UserPrincipalName $user -UsageLocation GB  
       $O365AllFac = New-MsolLicenseOptions -AccountSkuId  contoso:OFFICESUBSCRIPTION_FACULTYY -DisabledPlans SWAY
       Set-MsolUserLicense -UserPrincipalName $user -AddLicenses contoso:OFFICESUBSCRIPTION_FACULTY -LicenseOptions $O365AllFac  
...
function addSellectedProPlusStudentsUsr($user){
    try{
       ...
       Set-MsolUser -UserPrincipalName $user -UsageLocation GB
       $addO365AllStu = New-MsolLicenseOptions -AccountSkuId contoso:OFFICESUBSCRIPTION_STUDENT -DisabledPlans SWAY, OFFICE_FORMS_PLAN_2
       Set-MsolUserLicense -UserPrincipalName $user -AddLicenses contoso:OFFICESUBSCRIPTION_STUDENT -LicenseOptions $addO365AllStu
...
function addSellectedEduFacultyUsr($user){
  try{
        Write-Host "Adding all of 'Office 365 Education for faculty' for $($user)." -ForegroundColor Magenta
        Set-MsolUser -UserPrincipalName $user -UsageLocation GB
        $O365All = New-MsolLicenseOptions -AccountSkuId Contoso:STANDARDWOFFPACK_FACULTY -DisabledPlans SCHOOL_DATA_SYNC_P1, STREAM_O365_E3, TEAMS1, POWERAPPS_O365_P2, RMS_S_ENTERPRISE, YAMMER_EDU
...
function addSellectedEdcuStudentUsr($user){
    try{
       Write-Host "Adding all of  'Office 365 Education for students' for $($user)." -ForegroundColor Magenta
       Set-MsolUser -UserPrincipalName $user -UsageLocation GB
       $O365All = New-MsolLicenseOptions -AccountSkuId contoso:STANDARDWOFFPACK_STUDENT -DisabledPlans AAD_BASIC_EDU, SCHOOL_DATA_SYNC_P1, STREAM_O365_E3, TEAMS1, Deskless, FLOW_O365_P2, POWERAPPS_O365_P2, RMS_S_ENTERPRISE, YAMMER_EDU, MCOSTANDARD           
       Set-MsolUserLicense -UserPrincipalName $user -AddLicenses contoso:STANDARDWOFFPACK_STUDENT -LicenseOptions $O365All 
       Write-Host "Completed, added all of  'Office 365 Education for students' for $($user)." -ForegroundColor Magenta
   }
   catch{
